                ...
                if residue.get_resname() == "FAD":
                    ...
                    for atom in residue:
                        if atom.get_name() == "HN5":
                            hn5_connected = True
                            hn5_coords = atom.get_coord()  
                            break

                    if hn5_connected:
                        fad_candidates.append((residue, hn5_coords))

    if fad_candidates:
        for fad, hn5_coords in fad_candidates:
            n1_coords = None
            n5_coords = None
            classified_n1 = set()
            classified_n5 = set()
            ...
            if n1_coords is not None and n5_coords is not None:
                print(f"HN5 Coordinates for bifurcator {fad.get_full_id()}:", hn5_coords)
